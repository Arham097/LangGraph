...
def find_sentiment(state: ReviewState):
    prompt = f"Analyze the following review and determine if the sentiment is positive or negative: {state['review']}"
    output = structured_model_1.invoke(prompt).sentiment
...
def run_diagnosis(state: ReviewState):
    prompt = f"Diagnose the issue based on the following review: {state['review']}"
...
initial_state = {
    "review": "The product was horrible and the delivery was slow.",
}
